...
flowchart TB
    subgraph Entry[入口层]
        direction LR
        WEB[Web UI] & API[REST API] & WX[微信 Clawbot]
    end
    RT[AgentRuntime]
    subgraph Core[core — 领域内核（端口）]
        direction LR
        LOOP[ConversationLoop]
        PB[PromptBuilder]
        TR[ToolRegistry<br/>+ Dispatcher]
        MEM[Memory 端口]
        SK[Skill 端口]
        REV[BackgroundReview]
    end
    subgraph Infra[adapter — 适配 / 外部资源]
        direction LR
        MODEL[(OpenAI 兼容模型)]
        DB[(PostgreSQL<br/>pgvector+pg_trgm)]
        FS[文件存储]
        EMB[Embedding<br/>百炼 v4]
        MCPC[MCP<br/>stdio/HTTP]
    end

    WEB & API & WX --> RT --> LOOP
    LOOP --> PB & TR & MEM & SK & REV
    PB --> MODEL
    TR -. 桥接 .- MCPC
    MEM --> DB & FS & EMB
    SK --> FS
