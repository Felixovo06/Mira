...
| **长期记忆** | 文件即事实源 + DB 索引；pgvector 向量 + pg_trgm 词法混合检索；逻辑删除/封类 |
...
flowchart LR
    ...
    subgraph 写入
        W[write_memory] --> FILE[文件=事实源]
        FILE --> IDX[(memory_index)]
        FILE -. 异步 .-> GEN[生成 1536 维向量] --> IDX
    end
```

文件是事实源，DB 是可重建索引；删除走逻辑删除（archived 标记，不重建）。
...
docs/                        # 设计文档（产品/架构/数据/记忆/自我改善/接口验收/Hermes 对照）
```

设计文档见 [`docs/`](docs/)：产品需求、架构模块、数据设计、记忆系统、自我改善机制、核心接口与验收标准、Hermes 复刻对照。
...
5. **自我改善（Hermes 式）怎么做的？** 技能渐进式披露（索引进 prompt、正文按需）、单写者串行化保文件安全、去重再创建、隐私物理隔离（技能 schema 无用户事实字段）、按来源定 confidence；后台复盘双门控（工具数/信号词/轮数）异步提炼，Curator 归档/合并建议。
...
- **P3** 生态扩展：MCP 接入 ✅ · 常用工具包 ✅ · README/架构材料 ✅ · Web UI/微信入口 ✅（前序阶段已就绪）

构建与代码规范见 [`CLAUDE.md`](CLAUDE.md)。
